...
def TaxableIncomeTargetMethodWithSSI(TotalStandardIncome,TotalSS,MaxStandardIncome,SpecifiedIncome,MarriedOrOther):
    ...
    if MaxNonSSstandardIncome < 0.:
        ...
        TaxableSSincomeNoOtherIncome = TaxableSSconsolidated(0.,TotalSS,MarriedOrOther)
        ...
        if TaxableSSincomeNoOtherIncome < RemainingStandardIncomeRoom:
            MaxCapGains = np.round(ComputeMaxCapGains(TotalSS,RemainingStandardIncomeRoom,MarriedOrOther),2)
            # TaxableSSincome should now equal RemainingStandardIncomeRoom
            TaxableSSincome = np.round(TaxableSSconsolidated(0.+MaxCapGains,TotalSS,MarriedOrOther),2)
